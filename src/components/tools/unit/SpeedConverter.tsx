...
const UNITS:{name:string,factor:number}[]=[
  {name:"m/s",factor:1},{name:"km/h",factor:0.277778},
  {name:"mph",factor:0.44704},{name:"Knots (kt)",factor:0.514444},
  {name:"ft/s",factor:0.3048},{name:"Mach (sea level)",factor:340.29},
];
export function SpeedConverter(){
  const [val,setVal]=useState(""); const [from,setFrom]=useState(1); const [to,setTo]=useState(2);
  const n=parseFloat(val);
  const result=isNaN(n)?"":((n*UNITS[from].factor)/UNITS[to].factor).toFixed(6).replace(/\.?0+$/,"");
