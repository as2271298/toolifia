    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-5 rounded-2xl bg-slate-900 border border-slate-800 text-white shadow-xl">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-sm font-bold text-white block">Rewriting Style</span>
            <span className="text-[11px] text-slate-400">Select target tone and reading cadence</span>
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
          {[
            { id: "conversational", label: "💬 Natural Flow", desc: "Best for blogs & general text" },
            { id: "casual", label: "☕ Casual & Clear", desc: "Short punchy sentences" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-rose-500" /> Draft / AI-Generated Text Input
            </label>
          ...
          <textarea
            ...
            placeholder="Paste text generated by ChatGPT, Claude, Gemini, or any AI draft here to rewrite and improve flow..."
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-500" /> Humanized &amp; Refined Result
            </label>
            {output && (
              <button
                onClick={copyResult}
          ...
          <div className="w-full h-80 p-4 text-sm bg-slate-900 border border-slate-800 rounded-2xl overflow-y-auto text-slate-100 whitespace-pre-wrap leading-relaxed">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-3">
                ...
                <span className="text-xs text-slate-500">Enhancing sentence variety and natural phrasing</span>
              </div>
            ) : output ? (
              output
            ) : (
              <div className="flex flex-col items-center justify-center h-full text-slate-500 text-center p-6 space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Paste draft or AI text on the left and click <strong className="text-purple-400">Humanize Text</strong> to produce natural, human-like writing with improved flow.
                </p>
              </div>
            )}
          </div>
          ...
          {stats && (
            <div className="grid grid-cols-3 gap-3 p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center">
              <div>
                <div className="text-xs text-emerald-400 font-bold">Natural Flow</div>
                <div className="text-lg font-black text-white">{stats.humanScore}%</div>
              </div>
              <div>
                <div className="text-xs text-slate-400 font-medium">Readability</div>
                <div className="text-lg font-black text-emerald-400">{stats.readabilityGrade.split(" ")[0]}</div>
              </div>
              <div>
                <div className="text-xs text-slate-400 font-medium">Phrasing Refinements</div>
      ...
      <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs text-slate-400 flex items-start gap-2.5">
        ...
        <div>
          <span className="font-semibold text-slate-300">AI Performance Note: </span>
          Results vary depending on the input text and topic. Toolifia is engineered to produce natural, engaging writing with varied syntax, but detector scores cannot be guaranteed due to evolving third-party heuristics.
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 border-t border-slate-800">
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span>Restructures syntax, adjusts sentence burstiness &amp; enriches vocabulary</span>
        </div>
